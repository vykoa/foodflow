...
export default function ProducerHome() {
  ...
  const availableQty = Math.round(available.reduce((s, i) => s + i.quantity, 0));
  const atRisk = available.filter((i) => ["CRITICAL", "HIGH"].includes(i.waste_risk));
  const atRiskQty = Math.round(atRisk.reduce((s, i) => s + i.quantity, 0));
      ...
      <section>
        ...
        <div className="panel mt-2 overflow-hidden">
          <table className="data-table">
            <thead><tr><th>Food</th><th>Quantity</th><th>Risk</th></tr></thead>
            <tbody>
              {inventory.length === 0 && <tr><td colSpan={3} className="text-muted">Nothing listed yet.</td></tr>}
